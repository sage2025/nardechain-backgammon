...
class MainBoard extends Component {
    ...
    componentDidMount() {
        ...
        const players = { p1: "sage", p2: "drcyber"};
        ...
        socket.emit('start_game_room', 'sage', (error) => {
            console.log(error, "socket");
        });
    }

    componentDidUpdate() {
        socket.on('rolldice_fe', states => {
            //Save current state into 
            let flag = 0;
            for(var i=0; i<24; i++) {
                if(this.state.points[i].checkers === states.points[i].checkers && this.state.gameStatus > 11)
                    flag++;
            }
            if(flag < 24) {
                const moves = this.calculateCanMove(
                    this.getPointsWithoutActions(states.points),
                    states.dice,
                    states.p1IsNext,
                    this.state.grayBar
                );
                ...
                this.setState({
                    ...
                    currentPosition: states.currentPosition,
                    points: points,
                    dice: states.dice,
                    p1IsNext: states.p1IsNext,
                    // 
                });
            }
            console.log(states, flag, "1");
        })

        socket.on('dicemove_fe', states => {
            const grayBar = states.grayBar;
            const outSideBar = this.getOutSideBarWithoutActions(states.outSideBar);
            const dice = states.dice;
            let p1IsNext = states.p1IsNext;
            let gameStatus = states.gameStatus; //playing
            let showMenu = states.showMenu;
            //get points without actions
            let flag = 0;
            
            for(var i=0; i<24; i++) {
                if(this.state.points[i].checkers === states.points[i].checkers)
                    flag++;
            }
            if(flag < 24) {
                let points = this.getPointsWithoutActions(states.points);
                ...
                const currentPosition = states.currentPosition + 1;
                ...
                this.setState({
                    ...
                    grayBar: grayBar,
                    outSideBar: outSideBar,
                    movingChecker: states.movingChecker,
                    showMenu: showMenu,
                })
        ...
        socket.on('undo_fe', states => {
            console.log(states, "states");
            const history = states.history;
            const newPosition = states.currentPosition;
            ...
            const moves = this.calculateCanMove(states.history[newPosition].points, dice, p1IsNext, grayBar);
            const points = moves.points;
            const gameStatus = moves.gameStatus;
    ...
    rollDiceHandler = () => {
        ...
        const states = { gameStatus: gameStatus, history: history, currentPosition: currentPosition, points: points, dice: dice, p1IsNext: p1IsNext };
        socket.emit('rolldice', states, (error) => {
            console.log(error);
        })
        console.log("______________", states)
    }
    ...
    receiveCheckerHandler = (die) => {
        ...
        console.log(states, "player")
        socket.emit('dicemove', states, (error) => {
            console.log(error);
        } )
    ...
    render() {
        ...
        console.log(this.props.info, "gameboardprops")
